...
function loadState() {
  try {
    ...
    if (fs.existsSync(MOTOR2_FILE)) return JSON.parse(fs.readFileSync(MOTOR2_FILE, "utf8"));
  } catch (_) {}
  return {};
}
...
function saveState() {
  try {
    ...
    for (const [tid, d] of Object.entries(global.motorData2 || {})) {
      toSave[tid] = { status: d.status, message: d.message, time: d.time };
    }
...
function startInterval(api, threadID) {
  ...
  d.interval = setInterval(() => {
    ...
    if (Date.now() - lastActive < d.time * 2) {
      Promise.resolve(botApi.sendMessage(d.message, threadID)).then(() => {
        _failCounts[threadID] = 0;
      }).catch((err) => {
        const msg = String(err && (err.message || err)).toLowerCase();
        if (msg.includes("mqtt") || msg.includes("not connected")) return;
        const isDeadThread = msg.includes("no message_thread") || msg.includes("not a participant") || msg.includes("you are not a member");
        if (isDeadThread) {
          _failCounts[threadID] = (_failCounts[threadID] || 0) + 1;
          if (_failCounts[threadID] >= FAIL_THRESHOLD) {
            if (d.interval) { clearInterval(d.interval); d.interval = null; }
            d.status = false; delete _failCounts[threadID]; saveState();
          }
        }
      });
    }
...
module.exports.onLoad = function ({ api }) {
  ...
  for (const [tid, d] of Object.entries(saved)) {
    global.motorData2[tid] = { status: d.status || false, message: d.message || null, time: d.time || null, interval: null };
    if (d.status && d.message && d.time) startInterval(api, tid);
  }
};
...
module.exports.run = async function ({ api, event, args, permssion }) {
  ...
  if (args[0] === "وقت") {
    ...
    else return api.sendMessage("⚠️ استخدم s أو m.", threadID, messageID);
    if (ms < 5000) return api.sendMessage("⚠️ الحد الأدنى 5 ثواني.", threadID, messageID);
    data.time = ms; saveState();
  ...
  if (args[0] === "تفعيل") {
    ...
    if (!data.message) return api.sendMessage("⚠️ لم تحدد الرسالة.\nاستخدم: محرك2 رسالة [النص]", threadID, messageID);
    if (!data.time) return api.sendMessage("⚠️ لم تحدد الوقت.\nاستخدم: محرك2 وقت [30s أو 0.5m]", threadID, messageID);
    data.status = true; _failCounts[threadID] = 0; startInterval(api, threadID); saveState();
    return api.sendMessage(`✅ تم تفعيل المحرك الذكي.\n📝 "${data.message}"\n⏱ كل: ${data.time / 1000}s\n🔔 يرسل فقط عند وجود نشاط`, threadID, messageID);
  }
  if (args[0] === "ايقاف") {
    ...
    data.status = false; delete _failCounts[threadID]; saveState();
    return api.sendMessage("🔴 تم إيقاف المحرك.", threadID, messageID);
  }
  if (args[0] === "حالة") {
    return api.sendMessage(`📊 حالة المحرك الذكي\n\nالحالة: ${data.status ? "✅ مفعّل" : "🔴 موقوف"}\nالرسالة: ${data.message || "غير محددة"}\nالوقت: ${data.time ? data.time / 1000 + "s" : "غير محدد"}`, threadID, messageID);
  }
