...
async function searchYouTube(query) {
  ...
  if (ytKey) {
    const res = await axios.get(YT_SEARCH_API, { params: { part: "snippet", q: query, type: "video", maxResults: 1, key: ytKey }, timeout: 10000 });
    const item = res.data.items?.[0];
    if (item) return { url: `https://www.youtube.com/watch?v=${item.id.videoId}`, title: item.snippet.title };
  }
  const searchRes = await axios.get(`https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`, { headers: { "User-Agent": "Mozilla/5.0" }, timeout: 10000 });
  const match = searchRes.data.match(/"videoId":"([^"]+)","title":\{"runs":\[\{"text":"([^"]+)"/);
  if (match) return { url: `https://www.youtube.com/watch?v=${match[1]}`, title: match[2] };
...
async function downloadMp3(videoUrl) {
  ...
  try {
    const res = await axios.post(COBALT_API, { url: videoUrl, downloadMode: "audio", audioFormat: "mp3", filenameStyle: "basic" }, { headers: { "Accept": "application/json", "Content-Type": "application/json" }, timeout: 30000 });
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!query) return api.sendMessage("🎵 أرسل اسم الأغنية أو رابط يوتيوب\nمثال: song faded alan walker", threadID, messageID);
  ...
  try {
    let videoUrl = query, title = query;
    const isYtLink = /youtu(be\.com|\.be)\//i.test(query);
    if (!isYtLink) { const result = await searchYouTube(query); videoUrl = result.url; title = result.title; }
    ...
    if (!fs.existsSync(outPath) || fs.statSync(outPath).size < 10000) throw new Error("الملف الصوتي فارغ أو تالف");
    api.setMessageReaction("✅", messageID, () => {}, true);
    return api.sendMessage({ body: `🎵 ${title}\n🔗 ${videoUrl}`, attachment: fs.createReadStream(outPath) }, threadID, () => { try { fs.unlinkSync(outPath); } catch (_) {} }, messageID);
  } catch (e) {
    api.setMessageReaction("❌", messageID, () => {}, true);
