...
function loadNickState() {
  try {
    ...
    if (fs.existsSync(NICK_STATE_FILE)) return JSON.parse(fs.readFileSync(NICK_STATE_FILE, "utf8"));
  } catch (_) {}
  return {};
}
...
function saveNickState() {
  try {
    ...
    for (const [tid, cfg] of Object.entries(global.nickPersist || {})) {
      if (cfg && cfg.nickname) out[tid] = { nickname: cfg.nickname, intervalMs: cfg.intervalMs || DEFAULT_INTERVAL };
    }
...
function startProtection(api, threadID, nickname, intervalMs) {
  ...
  const state = { nickname, intervalMs, participantIDs: [], index: 0, cycleCount: 0, active: true, intervalId: null, failCount: 0 };
  global.nickProtect[threadID] = state;
  const botApi = global._botApi || api;
  botApi.getThreadInfo(threadID, (err, threadInfo) => {
    if (!state.active) return;
    if (err || !threadInfo?.participantIDs?.length) { _clearProtection(threadID); return; }
    ...
    state.intervalId = setInterval(async () => {
      ...
      if (state.index >= state.participantIDs.length) {
        state.cycleCount++; state.index = 0; state.failCount = 0;
        try { const fresh = await currentApi.getThreadInfo(threadID); if (fresh?.participantIDs?.length) state.participantIDs = fresh.participantIDs; } catch (_) {}
        return;
      }
      const userID = state.participantIDs[state.index];
      try {
        await new Promise((resolve) => {
          try { const r = currentApi.changeNickname(state.nickname, threadID, userID, () => resolve()); if (r && typeof r.catch === "function") r.catch(() => resolve()); } catch (_) { resolve(); }
        });
        state.failCount = 0;
      } catch (_) {}
      state.index++;
...
function _clearProtection(threadID) {
  ...
  if (old) { old.active = false; if (old.intervalId) { clearInterval(old.intervalId); old.intervalId = null; } delete global.nickProtect[threadID]; }
  if (global.nickSessions?.has(threadID)) { clearInterval(global.nickSessions.get(threadID)); global.nickSessions.delete(threadID); }
}
...
module.exports.config = {
  ...
  description: "حماية كنيات الأعضاء — تطبيق مستمر ومتكرر",
...
module.exports.onLoad = function ({ api }) {
  ...
  for (const [threadID, cfg] of Object.entries(saved)) {
    ...
    if (!global.nickProtect[threadID]?.active) startProtection(api, threadID, nickname, intervalMs);
  }
};
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (type === "ايقاف") {
    ...
    if (!isActive && !global.nickPersist?.[threadID]) return api.sendMessage("⚠️ لا توجد حماية كنيات نشطة.", threadID, messageID);
  ...
  if (type === "وقت") {
    const input = args[1];
    if (!input) return api.sendMessage("📌 الاستخدام: كنيات وقت [السرعة]\nأمثلة: كنيات وقت 3s | كنيات وقت 0.5m", threadID, messageID);
    const ms = parseInterval(input);
    if (!ms || ms < MIN_INTERVAL) return api.sendMessage(`⚠️ الحد الأدنى ${MIN_INTERVAL / 1000} ثواني.`, threadID, messageID);
    ...
    if (state?.active) {
      ...
      return api.sendMessage(`✅ تم تحديث السرعة إلى ${ms / 1000}s لكل عضو.`, threadID, messageID);
    } else {
      ...
      return api.sendMessage(`✅ تم حفظ السرعة: ${ms / 1000}s لكل عضو.`, threadID, messageID);
    }
  }
  ...
  if (type === "حالة") {
    const state = global.nickProtect?.[threadID];
    if (state?.active) {
      return api.sendMessage(`🛡️ حماية الكنيات نشطة\n\n📝 الكنية: "${state.nickname}"\n⏱️ السرعة: ${(state.intervalMs / 1000).toFixed(1)}s\n👥 الأعضاء: ${state.participantIDs?.length || 0}\n🔄 الدورة: ${(state.cycleCount || 0) + 1}\n\nللإيقاف: كنيات ايقاف`, threadID, messageID);
    } else {
      const cfg = global.nickPersist?.[threadID];
      return api.sendMessage(`🔴 حماية الكنيات غير نشطة\n📝 الكنية المحفوظة: ${cfg?.nickname || "غير محددة"}\n⏱️ السرعة المحفوظة: ${((cfg?.intervalMs || DEFAULT_INTERVAL) / 1000).toFixed(1)}s\n\nلتشغيلها: كنيات تشغيل [الكنية]`, threadID, messageID);
    }
  }

  if (type === "تشغيل") {
    const nickname = args.slice(1).join(" ").trim();
    if (!nickname) return api.sendMessage("📌 الاستخدام: كنيات تشغيل [الكنية المطلوبة]", threadID, messageID);
    const existing = global.nickProtect?.[threadID];
    if (existing?.active) return api.sendMessage(`⚠️ حماية الكنيات تعمل بالفعل بكنية: "${existing.nickname}"\nأوقفها أولاً: كنيات ايقاف`, threadID, messageID);
  ...
  return api.sendMessage("📌 أوامر الكنيات:\n• كنيات تشغيل [الكنية]\n• كنيات ايقاف\n• كنيات وقت [3s / 0.5m]\n• كنيات حالة", threadID, messageID);
};
