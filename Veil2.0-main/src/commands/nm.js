...
function _parseRandRange(str) {
  if (!str || !str.trim()) return { min: 30, max: 120 };
  ...
  config: {
    ...
    guide: {
      en: "{pn} [اسم] — تفعيل القفل\n" +
          ...
          "{pn} time r [نطاق] — وقت عشوائي (مثال: r 30-120)\n" +
          "{pn} status — الحالة"
    }
  },
  ...
  onStart: async function({ api, event, args, message }) {
    ...
    if (sub === "time") {
      ...
      if (rawTime.toLowerCase().startsWith("r")) {
        ...
        const lock = global._nmLocks[tid] || {};
        global._nmLocks[tid] = { ...lock, minDelay: range.min, maxDelay: range.max };
        const d = load(); if (!d[tid]) d[tid] = {}; Object.assign(d[tid], { minDelay: range.min, maxDelay: range.max }); save(d);
        if (global._nmLocks[tid].active) startTimer(api, tid);
        return message.reply(`🎲 وقت التجديد عشوائي: ${range.min}–${range.max} ثانية`);
      }
      const minDelay = parseInt(args[1]) || 30;
      const maxDelay = Math.max(parseInt(args[2]) || minDelay, minDelay);
      const lock = global._nmLocks[tid] || {};
      global._nmLocks[tid] = { ...lock, minDelay, maxDelay };
      const d = load(); if (!d[tid]) d[tid] = {}; Object.assign(d[tid], { minDelay, maxDelay }); save(d);
      if (global._nmLocks[tid].active) startTimer(api, tid);
