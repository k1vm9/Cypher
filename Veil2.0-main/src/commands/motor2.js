...
const { motor2 } = require("../engine/motorPersist");
...
module.exports.onLoad = function ({ api }) {
  ...
  if (!global.__motorWatchdog2) {
    global.__motorWatchdog2 = setInterval(() => {
      try {
        const { isActiveLoop } = require("../engine/motorSafeSend");
...
module.exports.run = async function ({ api, event, args, permssion }) {
  ...
  if (args[0] === "ايقاف") {
    ...
    data.status = false;
    try {
      const { stopMotorLoop } = require("../engine/motorSafeSend");
  ...
  if (args[0] === "حالة") {
    const { getLoopStats } = require("../engine/motorSafeSend");
