'use strict';
/**
 * song.js — تحميل أغاني من يوتيوب  v7.0 (ZAO edition)
 * ==========================================
...
const CACHE_DIR        = path.join(os.tmpdir(), 'veil-song');
...
async function _playdlDl(videoUrl, outPath) {
  ...
  await new Promise((resolve, reject) => {
    ...
    stream.stream.on('data', (chunk) => {
      total += chunk.length;
      if (total > MAX_BYTES) stream.stream.destroy(new Error('file too large'));
    });
...
async function _download(url, outPath) {
  ...
  if (len && len > MAX_BYTES) { res.data.destroy(); throw new Error('file too large'); }
...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    ...
    if (seconds && seconds > MAX_DURATION_SEC) throw new Error('الأغنية طويلة جداً (أكثر من 20 دقيقة)');
    ...
    let info = null, usedDirect = false;
    ...
    if (!fs.existsSync(outPath) || fs.statSync(outPath).size < 8000) throw new Error('الملف الصوتي فارغ أو تالف');
    if (fs.statSync(outPath).size > MAX_BYTES) throw new Error('file too large');

    api.setMessageReaction('✅', messageID, () => {}, true);

    return api.sendMessage(
      { body: `🎵 ${title}\n🔗 ${videoUrl}`, attachment: fs.createReadStream(outPath) },
      threadID,
