...
const runtimePersist = require("../engine/runtimePersist");

const CONFIG_PATH = path.join(process.cwd(), "config.json");

function loadSettings() {
  const cfg = fs.readJsonSync(CONFIG_PATH);
  cfg.adminBot = (cfg.adminBot || []).map(String);
  return cfg;
}

function saveSettings(cfg) {
  atomicWriteJsonSync(CONFIG_PATH, cfg, { spaces: 2 });
  if (global.config) global.config.adminBot = cfg.adminBot;
...
module.exports.run = async function ({ api, event, args, permssion }) {
  ...
  try {
    ...
    return api.sendMessage("❌ فشل قراءة config.json", threadID, messageID);
  }
  ...
  if (sub === "add" || sub === "-a") {
    ...
    try {
      saveSettings(cfg);
      runtimePersist.setAdmins(cfg.adminBot);
    } catch (e) {
    ...
    msg = (msg.trim() || "⚠️ لم يتم أي تغيير.");
    msg += runtimePersist.persistHint(cfg.adminBot);
