...
function _parseRandRange(str) {
  if (!str || !str.trim()) return { min: 60, max: 300 };
...
module.exports = {
  config: {
    ...
    guide: { en: "{pn} [رسالة] [min] [max] — تفعيل\n{pn} [رسالة] r [نطاق] — وقت عشوائي (مثال: r 60-300)\n{pn} off — إيقاف\n{pn} status — الحالة" }
  },
  ...
  onStart: async function({ api, event, args, message }) {
    ...
    if (!sub || sub === "status") {
      ...
      const tStr = td.minSeconds === td.maxSeconds
        ? `${td.minSeconds}s` : `🎲 ${td.minSeconds}–${td.maxSeconds}s`;
    ...
    // /angel [رسالة] [min] [max] | [رسالة] r [نطاق]
    const rArg      = args.find(a => /^r(\d|-|$)/i.test(a));
    const textParts = args.filter(a => !/^\d+$/.test(a) && a.toLowerCase() !== "on" && !/^r(\d|-|$)/i.test(a));
    const msg       = textParts.join(" ").trim() || data[tid]?.message || "🌸 مرحباً!";
    let minS, maxS;
    if (rArg) {
      const rangeStr = rArg.slice(1).trim() ||
        (args[args.indexOf(rArg) + 1] || "");
      const range = _parseRandRange(rangeStr);
      if (!range) return message.reply(
        "⚠️ صيغة خاطئة. أمثلة:\n/angel رسالتك r\n/angel رسالتك r 60-300\n/angel رسالتك r 1m-5m\n(الحد الأدنى 5s، الأكبر > الأصغر)"
      );
      minS = range.min; maxS = range.max;
    } else {
      const nums = args.filter(a => /^\d+$/.test(a));
      minS = parseInt(nums[0]) || 60;
      maxS = Math.max(parseInt(nums[1]) || minS, minS);
    }
    ...
    const timeStr = minS === maxS ? `${minS}s` : `🎲 ${minS}–${maxS}s`;
    message.reply(
      `✅ تم تفعيل Angel v5\n` +
      `📝 "${msg}"\n` +
      `⏱ كل ${timeStr}\n` +
