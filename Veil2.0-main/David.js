/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║              Veil — DjamelBot Engine (المحرك الرئيسي)           ║
 * ║           Copyright © 2025 DJAMEL — All rights reserved        ║
 * ║                    Version 2.0 | Engine: Veil                  ║
 * ╚══════════════════════════════════════════════════════════════════╝
 */
"use strict";
...
async function startBot() {
  ...
  function tryLogin() {
    ...
    DjamelFCA(cookies, { userAgent: UA }, async (err, api, extras) => {
      ...
      // اسم البوت
      let botName = config.botName || "Veil";
      try {
      ...
      const _cmdCount = (() => { let n = 0; for (const [k, v] of global.GoatBot.commands) if (String(v?.config?.name || "").toLowerCase() === k) n++; return n; })();
      console.log(`  │  ${chalk.yellow("Commands:")} ${chalk.white(String(_cmdCount).padEnd(35))}│`);
