...
module.exports = {
  ...
  // ── Command handler ──────────────────────────────────────────────────────
  run: async function ({ api, event, args, Threads, message }) {
    ...
    if (sub === 'add') {
      if (!isAdmin) return api.sendMessage('⚠️ Only bot admins can add blocked words.', threadID);
      ...
      return api.sendMessage(msg || '⚠️ No changes made.', threadID);
    }

    if (sub === 'del' || sub === 'delete' || sub === 'remove') {
      if (!isAdmin) return api.sendMessage('⚠️ Only bot admins can remove blocked words.', threadID);
      const rawWords = args.slice(1).join(' ').split(/[,|]/).map(w => w.trim().toLowerCase()).filter(Boolean);
      if (!rawWords.length) return api.sendMessage('⚠️ No words provided.', threadID);
      ...
      return api.sendMessage(msg || 'No changes.', threadID);
    }

    if (sub === 'list' || sub === 'all') {
      if (!stored.words.length) return api.sendMessage('📑 Blocked words list is empty.', threadID);
      const show = args[1] === 'show'
        ? stored.words.join(', ')
        : stored.words.map(hideWord).join(', ');
      return api.sendMessage(`📑 Blocked words (${stored.words.length}):\n${show}`, threadID);
    }

    if (sub === 'on') {
      if (!isAdmin) return api.sendMessage('⚠️ Only bot admins can enable this feature.', threadID);
      stored.enabled = true;
      await _saveBadWords(Threads, threadID, tData, stored);
      return api.sendMessage('✅ Bad words filter enabled.', threadID);
    }

    if (sub === 'off') {
      if (!isAdmin) return api.sendMessage('⚠️ Only bot admins can disable this feature.', threadID);
      stored.enabled = false;
      await _saveBadWords(Threads, threadID, tData, stored);
      return api.sendMessage('🔓 Bad words filter disabled.', threadID);
    }

    if (sub === 'unwarn') {
      if (!isAdmin) return api.sendMessage('⚠️ Only bot admins can clear warnings.', threadID);
      let targetID = Object.keys(event.mentions || {})[0]
        || args[1]
        || event.messageReply?.senderID;
      if (!targetID || isNaN(targetID)) return api.sendMessage('⚠️ Tag a user or provide their ID.', threadID);
      targetID = String(targetID);
      if (!stored.violations[targetID]) return api.sendMessage(`⚠️ User ${targetID} has no recorded violations.`, threadID);
      delete stored.violations[targetID];
      await _saveBadWords(Threads, threadID, tData, stored);
      return api.sendMessage(`✅ Cleared violation record for user ${targetID}.`, threadID);
    }
