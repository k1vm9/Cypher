...
module.exports.run = async function ({ api, event }) {
  ...
  if (sub === 'on') {
    let info;
    try { info = await api.getThreadInfo(threadID); } catch (_) {}
  ...
  if (sub === 'set') {
    ...
    try { await api.changeNickname(nickname, threadID, targetID); } catch (_) {}
...
module.exports.handleEvent = async function ({ api, event }) {
  try {
    ...
    let botID;
    try { botID = String(api.getCurrentUserID()); } catch (_) { botID = ''; }
    ...
    try { await api.changeNickname(saved, threadID, participant_id); } catch (_) {}
  } catch (_) {}
};
